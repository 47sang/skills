...
def normalize_requested_path(repo_root: Path, raw_path: str) -> str:
    ...
    try:
        ...
        raise RuntimeError(f"路径在仓库外部：{raw_path}") from error
...
def parse_conflict_hunks(lines: list[str], context: int) -> tuple[list[dict[str, object]], str | None]:
    ...
    while index < len(lines):
        ...
        if index >= len(lines) or lines[index] != "=======":
            return hunks, f"未终止的冲突，起始于第 {start_index + 1} 行"
...
def render_summary_text(repo_root: Path, reports: list[dict[str, object]]) -> str:
    lines = [f"仓库：{repo_root}", f"冲突文件数：{len(reports)}"]
    for report in reports:
        stages = ",".join(str(stage) for stage in report["stages"])
        lines.append(
            f"- {report['path']} | 类型={report['conflict_type']} | 阶段={stages} | 块数={report['marker_hunks']}"
        )
        if report["parse_error"]:
            lines.append(f"  解析错误：{report['parse_error']}")
    lines.append("使用 --file <路径> 查看压缩的块详情，或使用 --all 查看所有文件")
    return "\n".join(lines)
...
def render_detail_text(
    ...
    lines = [
        f"== {report['path']} ==",
        f"类型：{report['conflict_type']}",
        f"阶段：{', '.join(str(stage) for stage in report['stages'])}",
    ]
    parse_error = report["parse_error"]
    if parse_error:
        lines.append(f"解析错误：{parse_error}")

    hunks = report["hunks"]
    if hunks:
        lines.append(f"块数：{len(hunks)}")
        for index, hunk in enumerate(hunks, start=1):
            ...
            lines.extend(
                [
                    "",
                    f"[块 {index}] 当前行 {hunk['start_line']}-{hunk['end_line']}",
                    *section_lines("之前", truncate_lines(list(hunk["before_context"]), max_lines)),
                    *section_lines(
                        f"我们的 ({hunk['ours_label']})",
            ...
            lines.extend(
                [
                    *section_lines(
                        f"他们的 ({hunk['theirs_label']})",
                        truncate_lines(theirs, max_lines),
                    ),
                    *section_lines("我们与他们的差异", diff),
                    *section_lines("之后", truncate_lines(list(hunk["after_context"]), max_lines)),
                ]
            )
        return "\n".join(lines)
    ...
    lines.append("块数：0")
    lines.append("索引预览：")
    lines.extend(section_lines("我们的", preview["ours"]))
    lines.extend(section_lines("基础", preview["base"]))
    lines.extend(section_lines("他们的", preview["theirs"]))
    if "ours_vs_theirs_diff" in preview:
        lines.extend(section_lines("我们与他们的差异", preview["ours_vs_theirs_diff"]))
    return "\n".join(lines)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="汇总并提取压缩的合并冲突上下文。"
    )
    parser.add_argument("--repo", default=".", help="目标仓库内的路径。")
    parser.add_argument(
        "--file",
        action="append",
        default=[],
        help="要详细检查的冲突文件。重复使用以检查多个文件。",
    )
    parser.add_argument(
        "--all",
        action="store_true",
    ...
    if args.all and args.file:
        parser.error("--all 不能与 --file 组合使用")
    if args.context < 0:
        parser.error("--context 必须是非负数")
    if args.max_lines <= 0:
        parser.error("--max-lines 必须是正数")
    ...
    if not reports:
        message = json.dumps({"repo_root": str(repo_root), "conflicted_files": []}, indent=2) if args.json else f"仓库：{repo_root}\n冲突文件数：0"
        print(message)
        return 0
    ...
    if args.all:
        ...
        if missing:
            for path in missing:
                print(f"错误：未找到冲突文件：{path}", file=sys.stderr)
            return 2
